...
class PostgreSQLManager(BaseDatabaseManager):
    """Enhanced PostgreSQL database manager with circuit breaker and resilience features"""
    ...
    def __init__(self, config_path: str = None, enable_circuit_breaker: bool = True,
        ...
        if enable_circuit_breaker:
            cb_config = CircuitBreakerConfig(
                ...
                expected_exception=(sqlalchemy.exc.DatabaseError, psycopg2.Error, DatabaseError),
                timeout=30.0
            )
            self.circuit_breaker = create_circuit_breaker(
                f"postgresql_{self.component_name}", 
                cb_config
            )
        else:
    ...
    def connect(self) -> bool:
        """Connect to PostgreSQL database with enhanced resilience"""
        try:
            with self._connection_lock:
                ...
                self.logger.info(f"Connected to PostgreSQL database with connection pool")
                return True
    ...
    def disconnect(self):
        """Disconnect from PostgreSQL with cleanup"""
        with self._connection_lock:
            if self.engine:
    ...
    def _execute_with_circuit_breaker(self, operation_func, *args, **kwargs):
        ...
        if self.enable_circuit_breaker and self.circuit_breaker:
            try:
                ...
                self.logger.warning(f"PostgreSQL circuit breaker open: {e}")
                raise DatabaseError(
    ...
    def execute_query(self, query: str, params: Dict = None) -> Dict[str, Any]:
        """Execute PostgreSQL query with enhanced error handling and circuit breaker protection"""
        def _execute():
            ...
            except (sqlalchemy.exc.DatabaseError, psycopg2.Error) as e:
                self.logger.error(f"PostgreSQL query execution failed: {e}")
